...
os.makedirs("post-processing/coverage", exist_ok=True)

for i in range(0,len(samples)):
    os.makedirs("post-processing/%s" % conditions[i], exist_ok=True)
    os.system("ln -s %s/results_pipeline/%s/peak_calling_%s/%s_peaks_rmBlacklist.narrowPeak post-processing/%s" \
      % (baseDir, samples[i], genomeBuild, samples[i], conditions[i]))
    os.system("ln -s %s/results_pipeline/%s/aligned_%s_exact/%s_shift.bed post-processing/coverage"\
...
rule all:
    input:
        expand("{sample}_shift.fixed.bed", sample = samples),
        expand("post-processing/{condition}/{sample}_peaks_rmBlacklist.fixed.narrowPeak",
          sample = samples, condition = conditions)

rule remove_unmapped_contigs:
    input:
        "post-processing/{condition}/{sample}_peaks_rmBlacklist.narrowPeak"
    output:
        "post-processing/{condition}/{sample}_peaks_rmBlacklist.fixed.narrowPeak"
    shell:
        """
        """
